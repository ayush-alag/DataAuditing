#!/bin/bash
#SBATCH --job-name=covidPrune    # create a short name for your job
#SBATCH --nodes=1                # node count
#SBATCH --ntasks=1               # total number of tasks across all nodes
#SBATCH --cpus-per-task=8        # cpu-cores per task (>1 if multi-threaded tasks)
#SBATCH --mem-per-cpu=2G         # memory per cpu-core (4G is default)
#SBATCH --time=0-02:00:00        # total run time limit (days-HH:MM:SS)
...
do
   ...
   experiment=$dataset"_sparsity"$sparsity
   python run_pruning.py --dataset $dataset --expt $experiment --sparsity $sparsity --mode $mode --epochs $epoch
